...
2. PARALLEL INVESTIGATION: Immediately ask BOTH the Telemetry Analyst and the FinOps Agent to assess the situation based on their specific domains.
...
RULES:
1. INVESTIGATION: You MUST first use `get_cloudwatch_metrics` to confirm if there is a resource spike on the underlying instances. 
...
5. VERIFICATION: You only use `run_synthetic_health_check` if the Supervisor explicitly asks you to verify the application status."""
...
RULES:
1. INVESTIGATION: You MUST first use `get_cloudwatch_metrics` to see if the infrastructure is experiencing heavy load.
